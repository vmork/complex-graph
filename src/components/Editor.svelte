<script lang="ts">
    ...
    import { workspaceExamples, defaultWorkspaceName, WorkspaceData } from "../workspace";
    ...
    document.addEventListener("click", e => {
        if (showExamples && !(e as any).target.closest("#examples-container") && !(e as any).target.closest("#tooltip-container")) {
            showExamples = false;
        }
    });
    ...
    <div id="tooltip-container">
        ...
        <div id="examples-container" transition:fade={{duration: 100}}>
            {#each Object.entries(workspaceExamples) as [name, example]}
                <button class:selected={name === currentExampleName}
...
<pre class:error={$compilationErrors.length > 0} 
    id="editor" contenteditable="true" 
    bind:this={preElem}
    on:input={handleInput} on:keydown={handleKeyDown}>{code}</pre>
